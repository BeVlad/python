...
class AcceptStorageError(AppError):
    def __init__(self, text):
        self.text = f"Unable to add product: \n {text}"
...
class Storage:
    ...
    def add(self, equipments):
        if not all([isinstance(equipment, OfficeEquipment) for equipment in equipments]):
            raise AddStorageError(f"This is not office equipment")
...
class OfficeEquipment:
    ...
    def __setattr__(self, key, value):
        if key in self.__required_props and not value:
            raise AttributeError(f"'{key}' most be true")
    ...
    def validate(cnt):
        if not isinstance(cnt, int):
            ValidateEquipmentError(f"'{type(cnt)}'; this should be a number")
...
class Xerox(OfficeEquipment):
    def __init__(self, **kwargs):
        super().__init__(eq_type='Samsung', **kwargs)


if __name__ == '__main__':
    storage = Storage()
    storage.add(Printer.create(4, vendor="Samsung", model="MF-4070D"))
    storage.add(Scanner.create(3, vendor="Xerox", model="D582"))
    storage.add(Scanner.create(2, vendor="Xerox", model="F123"))
    storage.add(Xerox.create(6, vendor="Cannon", model="HF-23"))
    print(storage)

    for idx, itm in storage.filter_by(department=None, vendor="Xerox", model="D582"):
        print(f"Reserved {itm} for 'IT department'")
        storage.transfer(idx, 'IT department')

    for idx, itm in storage.filter_by(department=None):
        print(idx, f"{itm} not divided into departments")

    print(storage)
    print("One device on debit")
    del storage[0]
    print(storage)
